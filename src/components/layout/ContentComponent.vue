  ...
  >
    <div id="master-card" class="w-[485px] h-[710px] mx-1" @click.stop>
      <MasterCardComponent
        ...
        :desc="
          filteredCivilisations[selectedCardIndex]?.historical_info[0]?.text
        "
        ...
        :civ_leader_effect="
          filteredCivilisations[selectedCardIndex]?.leader.trait.effect
        "
        :civ_units="filteredCivilisations[selectedCardIndex]?.unique_units"
        :civ_building="
          filteredCivilisations[selectedCardIndex]?.unique_buildings
        "
      />
    </div>
  </div>
  <div
    class="content relative flex flex-col align-top items-start sm:px-8 md:px-12 lg:px-20"
  >
...
const filteredCivilisations = computed(() => {
  return props.civilisations.filter((civ) => {
    ...
    let tiersMatch =
      ...
      selectedTiers.value.some(
        (selectedTier) => selectedTier.id === civ.tier_id,
      );
...
const selectedCardIndex = computed(() => {
  return filteredCivilisations.value.findIndex(
    (civ) => civ.id === selectedCard.value,
  );
});
